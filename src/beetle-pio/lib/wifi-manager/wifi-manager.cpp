    ...
    switch (modi) {
      ...
      case 1: {
        ...
        while (
          ...
          if (method == ERequestParsingMode::None && strcmp(buffer, "GET /connect?") == 0) {
#ifndef RELEASE
...
#ifndef RELEASE
            Serial.println("[wifi_manager] found ssid information");
#endif
          ...
          if (method == ERequestParsingMode::Password && buffer[cursor] == ' ') {
#ifndef RELEASE
            Serial.println("[wifi_manager] found password information");
#endif
            unsigned char offset = 0;
            const char * value = buffer + offset + field;
