...
std::optional<Events::EMessage> Events::update(uint32_t current_time) {
  ...
  switch (modi) {
    case 0: {
      ...
      active->_disconnected =
          WiFi.status() == WL_CONNECTED ? 0 : active->_disconnected + 1;
      ...
      // If we're no longer disconnected, but were previously, we've been
      // resumed.
      ...
      if (active->_disconnected > MAX_CONNECTION_INTERRUPTS) {
        log_e("wifi manager disonncted after %d attempts",
              active->_disconnected);
    ...
    case 1: {
      ...
      size_t stored_ssid_len =
          _checked_stored_values == false && _preferences.isKey("ssid")
              ? _preferences.getString("ssid", ssid, MAX_SSID_LENGTH)
              : 0;
      size_t stored_password_len =
          _checked_stored_values == false && _preferences.isKey("password")
              ? _preferences.getString("password", password,
                                       MAX_PASSWORD_LENGTH)
              : 0;

      if (_checked_stored_values == false) {
        _checked_stored_values = true;
        log_i("stored ssid len - %d (password %d)", stored_ssid_len,
              stored_password_len);
      }
      ...
      if (stored_ssid_len == 0 || stored_password_len == 0) {
        ...
        // If the server _also_ doesn't have an ssid or password available for
        // us, break
      ...
      // Move ourselves into the pending connection state. This will terminate
      // our
      // wifi server.
     ...
     * Connection Mode
     *
     * During this phase, we have an ssid + password ready, we just need to
     * attempt
    ...
    case 2: {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        size_t stored_password_len =
            _preferences.putString("password", pending->_password);
        log_d("stored ssid len %d, password len %d", stored_ssid_len,
              stored_password_len);
      ...
      // If we have seen too many frames without establishing a connection to
      // the
...
bool Events::PendingConfiguration::update(char *ssid, char *password) {
  ...
  // TODO: figure out how to decouple this so that consumers can provide their
  // own index html.
  // Currently, this is used to avoid the RAM cost associated with carrying
  // around the char[]
  ...
  while (
      client.connected() && cursor < SERVER_BUFFER_CAPACITY - 1 &&
    ...
    if (method == ERequestParsingMode::None &&
...
/**
 * When parsing the statusline of a request, this function will return the
 * character
 * that is expected to terminate a given parsing mode.
 */
inline char Events::PendingConfiguration::termination(
    ERequestParsingMode mode) {
  switch (mode) {
    case ERequestParsingMode::Network:
      return '&';
    case ERequestParsingMode::Password:
