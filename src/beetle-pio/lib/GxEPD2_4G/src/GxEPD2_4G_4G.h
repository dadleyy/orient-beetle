...
// from Waveshare.
// Requires HW SPI and Adafruit_GFX. Caution: these e-papers require 3.3V supply
// AND data lines!
//
...
class GxEPD2_4G_4G_R : public GxEPD2_4G_GFX_BASE_CLASS {
 public:
  GxEPD2_Type epd2;
#if ENABLE_GxEPD2_4G_GFX
  GxEPD2_4G_4G_R(GxEPD2_Type epd2_instance)
      : GxEPD2_4G_GFX_BASE_CLASS(epd2, GxEPD2_Type::WIDTH, GxEPD2_Type::HEIGHT),
        epd2(epd2_instance)
#else
  GxEPD2_4G_4G_R(GxEPD2_Type epd2_instance)
      : GxEPD2_4G_GFX_BASE_CLASS(GxEPD2_Type::WIDTH, GxEPD2_Type::HEIGHT),
        epd2(epd2_instance)
#endif
  {
  ...
  void drawPixel(int16_t x, int16_t y, uint16_t color) {
    ...
    if (color > 0) {
      ...
      if (color != GxEPD_WHITE) {
        uint32_t brightness =
            (uint32_t(color & 0xF800) + uint32_t((color & 0x07E0) << 5) +
             uint32_t((color & 0x001F) << 11));
        brb = uint8_t((brightness - 1) /
                      0xC000ul);  // GxEPD_LIGHTGREY is one too high
      }
  ...
  // power supply was kept
  // only relevant for b/w displays with fast partial update
  // reset_duration = 10 is default; a value of 2 may help with "clever" reset
  // circuit of newer boards from Waveshare
  // pulldown_rst_mode true for alternate RST handling to avoid feeding 5V
  // through RST pin
  ...
  void init(uint32_t serial_diag_bitrate, bool initial, uint16_t reset_duration,
  ...
  void fillScreen(uint16_t color)  // 0x0 black, >0x0 white, to buffer
  {
    uint32_t brightness =
        (uint32_t(color & 0xF800) + uint32_t((color & 0x07E0) << 5) +
         uint32_t((color & 0x001F) << 11));
    uint8_t brb = uint8_t((brightness - 1) /
                          0xC000ul);  // GxEPD_LIGHTGREY is one too high
  ...
  bool nextPage() {
    ...
    if (_using_partial_mode) {
      ...
      // Serial.print(_pw_y); Serial.print(", ");
      // Serial.print(_pw_w); Serial.print(", "); Serial.print(_pw_h);
      // Serial.print(") P"); Serial.println(_current_page);
      uint16_t page_ye =
      ...
      if (dest_ye > dest_ys) {
        ...
        // Serial.print(dest_ys); Serial.print(", ");
        // Serial.print(_pw_w); Serial.print(", "); Serial.print(dest_ye -
        // dest_ys); Serial.println(")");
        epd2.writeImage_4G(_buffer, 2, _pw_x, dest_ys, _pw_w,
                           dest_ye - dest_ys);
      } else {
        // Serial.print("writeImage("); Serial.print(_pw_x); Serial.print(", ");
        // Serial.print(dest_ys); Serial.print(", ");
        // Serial.print(_pw_w); Serial.print(", "); Serial.print(dest_ye -
        // dest_ys); Serial.print(") skipped ");
    ...
    {
      epd2.writeImage_4G(_buffer, 2, 0, page_ys, WIDTH,
  ...
  void drawPaged(void (*drawCallback)(const void*), const void* pv) {
    ...
    } else  // full update
    {
      ...
      for (_current_page = 0; _current_page < _pages; _current_page++) {
        ...
        fillScreen(GxEPD_WHITE);
        drawCallback(pv);
        epd2.writeImage(_buffer, 0, page_ys, WIDTH,
                        gx_uint16_min(_page_height, HEIGHT - page_ys));
      }
      if (epd2.panel == GxEPD2_4G::GDEW0154Z04) {  // GxEPD2_154c paged
                                                   // workaround: write color
                                                   // part
        for (_current_page = 0; _current_page < _pages; _current_page++) {
          uint16_t page_ys = _current_page * _page_height;
          fillScreen(GxEPD_WHITE);
  ...
  void drawGreyPixmap(const uint8_t pixmap[], int16_t depth, int16_t x,
      ...
      case 4: {
        ...
        for (int16_t j = 0; j < h; j++) {
          for (int16_t i = 0; i < w; i++) {
            ...
            if ((grey < 0xF0) && (grey >= 0xA0))
              grey = 0x80;  // light grey demo limit for 4bpp
            else if ((grey < 0xF0) && (grey > 0x00))
              grey = 0x40;  // dark grey
  ...
  void clearScreen(
      uint8_t value =
          0xFF)  // init controller memory and screen (default white)
  {
  ...
  void writeImage_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x, int16_t y,
  ...
  void writeImagePart(const uint8_t bitmap[], int16_t x_part, int16_t y_part,
                      ...
                      int16_t w, int16_t h, bool invert = false,
                      bool mirror_y = false, bool pgm = false) {
    epd2.writeImagePart(bitmap, x_part, y_part, w_bitmap, h_bitmap, x, y, w, h,
                        invert, mirror_y, pgm);
  }
  void writeImagePart_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x_part,
                         int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
                         int16_t x, int16_t y, int16_t w, int16_t h,
                         bool invert = false, bool mirror_y = false,
                         bool pgm = false) {
    epd2.writeImagePart_4G(bitmap, bpp, x_part, y_part, w_bitmap, h_bitmap, x,
  ...
  void writeImagePart(const uint8_t* black, const uint8_t* color,
    ...
    epd2.writeImagePart(black, color, x_part, y_part, w_bitmap, h_bitmap, x, y,
  ...
  void drawImage_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x, int16_t y,
                    int16_t w, int16_t h, bool invert = false,
                    bool mirror_y = false, bool pgm = false) {
  ...
  void drawImagePart_4G(const uint8_t bitmap[], uint8_t bpp, int16_t x_part,
                        int16_t y_part, int16_t w_bitmap, int16_t h_bitmap,
                        int16_t x, int16_t y, int16_t w, int16_t h,
                        bool invert = false, bool mirror_y = false,
                        bool pgm = false) {
    epd2.drawImagePart_4G(bitmap, bpp, x_part, y_part, w_bitmap, h_bitmap, x, y,
                          w, h, invert, mirror_y, pgm);
  }
 ...
 private:
  template <typename T>
  static inline void _swap_(T& a, T& b) {
    T t = a;
    a = b;
    b = t;
  };
...
class GxEPD2_4G_4G : public GxEPD2_4G_4G_R<GxEPD2_Type, page_height> {
 ...
 public:
  GxEPD2_4G_4G(GxEPD2_Type epd2_instance)
      : GxEPD2_4G_4G_R<GxEPD2_Type, page_height>(epd2_copy),
        epd2_copy(epd2_instance) {}
};
